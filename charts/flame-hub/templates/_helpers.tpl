...
{{/*
Refuse a clientUI.cookieDomain that reaches authup's own origin without a
clientUI.cookiePrefix to keep the two apart.

The Hub UI and authup's hosted consoles both persist their session under the
SAME cookie names (both run @authup/client-web-kit's store). They stay apart
only while their (name, domain, path) keys differ: authup scopes its records
to the sub-path it is served under, and the UI writes host-only records at
the root. Naming a domain opts the UI out of host-only - the browser then
delivers its records to every subdomain, so a value covering authup's host
puts two writers back on one key. Reads take the FIRST record of a name,
which is the older one, so each side can end up driving on - and
cleanup-revoking - the other's tokens. cookiePrefix (authup >= 1.0.0-beta.64)
namespaces the UI's cookie names apart from authup's own, so the two no
longer share a key even once the domain is widened.

Empty cookieDomain (the default) is host-only and always safe. A value is
only for sharing the session with a sibling host that must read the cookie
itself.
*/}}
...
{{- if not (.Values.clientUI.cookiePrefix | default "" | toString | trim) -}}
{{- fail (printf "clientUI.cookieDomain (%q) covers the authup host (%q), so the Hub UI session cookies are delivered into authup's own origin and collide with the console's records under the same names. Set clientUI.cookiePrefix to namespace the UI's cookies apart from authup's (needs authup >= 1.0.0-beta.64 / hub image >= 0.16.1), leave clientUI.cookieDomain empty (host-only cookies), or move authup to a host outside that domain." $domain $authupHost) -}}
{{- end -}}
{{- end -}}
{{- end -}}
{{- end -}}
